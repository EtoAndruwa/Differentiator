...
size_t print_tree_data(Tree* tree_struct, Node* node_ptr, const char* file_name)
{
    ...
    if(node_ptr != nullptr)
    {
        ...
        {
            fprintf(graph_txt, "\tnode_%p[shape = Mrecord, style=\"filled\" fillcolor=\"%s\", label =\" { <f0> left_child = %p } | {{<here> type = FUNCTION} | {value = %s \\n}} | { <f1> right_child = %p } \"];\n", node_ptr, PURP_BG_COLOR_DOT, node_ptr->left_child, node_ptr->value.text, node_ptr->right_child);
        }
        else
        {
